...
@section('content')
    <table class="table-auto w-full text-xs sm:text-sm md:text-base rounded-lg">
        ...
        @foreach($userHouses as $house)
            <tr>
                <td class="border-b border-slate-100 p-2 text-slate-500">{{ $house->user->name ?? ''}}</td>
                <td class="border-b border-slate-100 p-2 text-slate-500">{{ $house->user->email ?? ''}}</td>
                <td class="border-b border-slate-100 p-2 text-slate-500">{{ $house->type ?? '' }}</td>
            </tr>
        @endforeach
        </tbody>
    </table>
